...
import logging
from RedisBus import publish, subscribe
...
last_punct_word_index2 = {} # nothing like a temp fix that becomes permanent, get new regions quick fix for multi hosts last_punct_word_index3 = {}  # this and the below are for handle incoming
...
#                                 # }))
#
#                                 await publish("translation_output", {"lang": lang_code, "translated": updated_txt, "sessionID": session_id})
#                                 await publish("translation_output",{"lang": lang_code, "translated": updated_txt, "sessionID": f"{session_id} + 1" })
#
#
#                                 # # viewer
#                                 # await ws.send(json.dumps({  # send bullshit to the bullshit frontend
...
#
#                     await publish("translation_output",{"translated_Host_1":  updated_txt_Host_1, "translated_Host_2":  updated_txt_Host_2, "sessionID": session_id})
#
#                     # await ws.send(json.dumps({ # send bullshit to the bullshit frontend
#                     #     "source": "translate",
...
#             async for msg in subscribe("punctuate_input"):
#
#                 # msg is already a parsed dict (your RedisBus should return dict)
#                 session_msg_id = msg.get("session_id")
#
#                 if session_msg_id != session_id:
...
async def translator_worker():
    ...
    Single global worker:
      - subscribes to 'punctuate_input'
      - processes punctuation for all sessions
      - writes translations to DB
      - publishes results on 'translation_output'
    """
    logger.debug("Translator global worker starting; subscribing to 'punctuate_input'")

    async for raw in subscribe("punctuate_input"):
        # Normalize message to dict
        if isinstance(raw, (bytes, str)):
            try:
                msg = json.loads(raw.decode() if isinstance(raw, bytes) else raw)
            except Exception as e:
                logger.debug(f"Bad message on punctuate_input: {e} | raw={raw!r}")
                continue
        elif isinstance(raw, dict):
            msg = raw
        else:
            logger.debug(f"Unexpected message type on punctuate_input: {type(raw)}")
            continue
        ...
        if not session_id or not isinstance(payload, dict):
            logger.debug(f"Ignoring malformed message: {msg!r}")
            continue

        # -------------------------------------------------------
        # Determine session type and raw punctuated text
        # -------------------------------------------------------
        english_punct = payload.get("english_punctuated", "")

        if english_punct == "":
            # CoClient (dual host) mode
            Host_1_Punctuated = payload.get("Host_1_punctuated", "")
            Host_2_Punctuated = payload.get("Host_2_punctuated", "")

            if not Host_1_Punctuated and not Host_2_Punctuated:
                logger.debug(f"[{session_id}] Empty CoClient punctuate message; skipping")
                continue

            SessionType = "CoClient"
        else:
            # Single client mode
            SessionType = "Client"
            Host_1_Punctuated = None
            Host_2_Punctuated = None

        # -------------------------------------------------------
        # Compute new punctuated region(s)
        # -------------------------------------------------------
        with time_block(session_id, "handle_incoming", "get_region_punctuated"):
            if SessionType == "Client":
                region, new_index = get_new_region(english_punct, session_id, 3)
                if not region:
                    logger.debug(f"[{session_id}] No new region for Client; skipping")
                    continue

            else:  # CoClient
                region1, new_index_Host1 = get_new_region(Host_1_Punctuated, session_id, 3)
                region2, new_index_Host2 = get_new_region(Host_2_Punctuated, session_id, 4)
                if not region1 and not region2:
                    logger.debug(f"[{session_id}] No new region for CoClient; skipping")
                    continue
        ...
        with time_block(session_id, "handle_incoming", "db_read_again"):
            ...
            try:
                ...
                if SessionType == "Client":
                    ...
                    Host_1_Output = (session.Host1_out_transcript or "").strip()
                    Host_2_Output = (session.Host2_out_transcript or "").strip()
                    Host_1_Out_Lang = (session.Host1_lang_out or "es")
                    Host_2_Out_Lang = (session.Host2_lang_out or "en")

            finally:
                db.close()

        # -------------------------------------------------------
        # Translation: single-host
        # -------------------------------------------------------
        if SessionType == "Client":
            logger.debug(f"[{session_id}] Client handle_incoming start")
            ...
            for lang_code, is_active in targets.items():
                ...
                with time_block(session_id, "handle_incoming", f"translate_punct_{lang_code}"):
                    ...
                    # Redis-only output for websocket handler
                    await publish(
                        "translation_output",
                        {
            ...
            db = SessionLocal()
            try:
                session = get_or_create_session(db, session_id, "Translate_Client_Commit")
                session.translations = current_translations
                session.last_updated = datetime.utcnow()
                db.commit()
            finally:
            ...
            # Update word indices
            last_punct_word_index3[session_id] = new_index

        # -------------------------------------------------------
        # Translation: CoClient (dual host)
        # -------------------------------------------------------
        else:
            logger.debug(f"[{session_id}] CoClient handle_incoming start")

            translated_host_2_out = await translate_text(session_id, region1, Host_2_Out_Lang) if region1 else ""
            translated_host_1_out = await translate_text(session_id, region2, Host_1_Out_Lang) if region2 else ""

            updated_txt_Host_1 = replace_section(
                session_id, Host_1_Output, translated_host_1_out, CONTEXT_WORDS
            ) if translated_host_1_out else Host_1_Output

            updated_txt_Host_2 = replace_section(
                session_id, Host_2_Output, translated_host_2_out, CONTEXT_WORDS
            ) if translated_host_2_out else Host_2_Output
            ...
            await publish(
                "translation_output",
                {
                    ...
                    "source": "translate",
                    "payload": {
                        "translated_Host_1": updated_txt_Host_1,
                        "translated_Host_2": updated_txt_Host_2,
                    },
                },
            )
            ...
            db = SessionLocal()
            try:
                session = get_or_create_session(db, session_id, "Translate_CoClient_Commit")
                session.Host1_out_transcript = updated_txt_Host_1
                session.Host2_out_transcript = updated_txt_Host_2
                session.last_updated = datetime.utcnow()
                db.commit()
            ...
            # Update word indices
            last_punct_word_index3[session_id] = new_index_Host1
            last_punct_word_index4[session_id] = new_index_Host2
...
if __name__ == "__main__":
    try:
        asyncio.run(translator_worker())
    except KeyboardInterrupt:
        logger.info("Translator worker stopped by KeyboardInterrupt")